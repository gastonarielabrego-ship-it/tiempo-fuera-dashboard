  ...
  try {
    ...
    try {
      const tbl: any[] = await db.$queryRawUnsafe(
        `SELECT table_name FROM information_schema.tables WHERE table_name = 'Comida'`
      );
    ...
    if (search) {
      whereClause += ` AND ("dni" LIKE $${pIdx} OR "nombre" ILIKE $${pIdx})`;
    ...
    // Count total
    const countSql = `SELECT COUNT(*)::int as total FROM "Comida" ${whereClause}`;
    ...
    const datesRows: any[] = await db.$queryRawUnsafe(
      `SELECT DISTINCT "fecha" FROM "Comida" ORDER BY "fecha" DESC`
    );

    // Count summary
    const summaryRows: any[] = await db.$queryRawUnsafe(
      `SELECT COUNT(DISTINCT "dni")::int as trabajadores, COUNT(DISTINCT "fecha")::int as dias FROM "Comida"`
    );
    ...
    console.error('Comida API error:', error);
    return NextResponse.json({ error: 'Failed to fetch comida data', data: [], total: 0, page: 1, pageSize: 50, totalPages: 0, uniqueDates: [] }, { status: 500 });
  }
}